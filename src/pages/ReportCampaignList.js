...
export default function ReportCampaignList() {
    console.log("---> ReportCampaignList rendering ")
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold tracking-tight text-violet-800 p-8">
                Xem báo cáo thống kê các chiến dịch <i class='text-violet-600 bx bxs-report'></i>
            </h1>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
                {data.map((item) => (
                    <Link to={`/auth/ReportCampaign/${item.id}/${item.charityOrgId}`}
                        key={item.id}
